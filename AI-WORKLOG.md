...
- Candidate: asked for CI that runs the verify script and deploys to Render only if it passes. No Render account/service was created in this session (outside what's doable here) — documented the exact Render-side setup (build/start commands, which env vars must be set in Render's dashboard rather than committed, turning off Render's own auto-deploy so the CI gate is the only thing that can trigger a deploy) and flagged a real operational caveat: Render's default disk is ephemeral, so SQLite resets on every deploy/restart unless a persistent disk is attached — noted as an open item rather than silently glossed over, since deploying without addressing it would mean losing the seeded admin/demo data on every restart.
